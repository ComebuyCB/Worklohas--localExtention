  ...
  class ArticleBatch {
    constructor(options) {
      const def = {
        ...
        exportReplyLimit: 0,  // 匯出時每則留言最多取幾則回覆（range 預設值）
    ...
    // 重新爬 textarea 內所有文章與熱門留言，輸出 json 或 Word
    async exportFile(format) {
      ...
      const limits = {
        postLimit: Number(this.postLimitInput.value),
        replyLimit: Number(this.replyLimitInput.value),
      };
      ...
      try {
        ...
        format === 'json'
          ? this._download(`${filename}.json`, JSON.stringify({ exportedAt: new Date().toISOString(), articles }, null, 2), 'application/json')
          : this._download(`${filename}.doc`, this._docMarkup(articles), 'application/msword');
    ...
    _createPanel() {
      ...
      panel.innerHTML = `
        <div class="wl_batch--head">
          <strong class="wl_batchTitle">批次讀取文章</strong>
          <button class="wl_batchBtn js-batch-toggle" type="button">收合</button>
        </div>
        <div class="wl_batch--body">
          <form class="wl_batchForm js-batch-form">
            ...
            <textarea id="__lineToday_batchInput" class="wl_batchInput js-batch-input" rows="6" placeholder="https://today.line.me/tw/v3/article/xxxxxxx"></textarea>
            <button class="wl_batchBtn is_primary" type="submit">查詢</button>
          </form>
          <div class="wl_batchResult js-batch-result" hidden>
            ...
            <div class="wl_batchSettings">
              <label class="wl_batchRange">
                <span class="_label">留言數</span>
                <input class="js-batch-range js-batch-post-limit" type="range" min="0" max="50" step="5" value="${this.exportPostLimit}">
                <output class="_value">${this.exportPostLimit}</output>
              </label>
              <label class="wl_batchRange">
                <span class="_label">留言回覆數</span>
                <input class="js-batch-range js-batch-reply-limit" type="range" min="0" max="50" step="5" value="${this.exportReplyLimit}">
                <output class="_value">${this.exportReplyLimit}</output>
              </label>
            </div>
            <div class="wl_batchActions">
              <button class="wl_batchBtn js-batch-export" type="button" data-format="json">輸出 JSON</button>
              <button class="wl_batchBtn js-batch-export" type="button" data-format="doc">輸出 Word</button>
    ...
    _formatPost(post, article) {
      return { articleId: article.articleId, anchorId: article.anchorId, postId: post.id, author: post.author?.name ?? '', text: post.content?.text ?? '', likes: undefined, replyCount: undefined, replies: [] };
    }

    _formatReply(reply) {
      return { replyId: reply.id, author: reply.author?.name ?? '', text: reply.text ?? '', likes: reply.reactionStats?.count ?? 0 };
    }
    ...
    _renderTable() {
      ...
      this.articles.forEach((article) => {
        ...
        article.row.innerHTML = `
          <td><a href="${this._escapeHTML(article.url)}" target="_blank" rel="noopener noreferrer">${this._escapeHTML(article.url)}</a></td>
    ...
    _renderPosts(posts) {
      this.modalPosts.insertAdjacentHTML('beforeend', posts.map((post) => `
        <div class="wl_batchPost js-batch-post" data-post-id="${this._escapeHTML(post.postId)}">
          <strong class="_author">${this._escapeHTML(post.author)}</strong>
          <p class="_text">${this._escapeHTML(post.text)}</p>
          <div class="wl_batchPostMeta js-batch-meta">${this._metaMarkup(post)}</div>
          <div class="wl_batchReplies js-batch-replies"></div>
        </div>
      `).join(''));
    }
    ...
    _metaMarkup(post) {
      ...
      return `
        <span class="wl_batchStat${post.likes ? '' : ' is_zero'}">${ICON_LIKE}${post.likes ?? '-'}</span>
        <button class="wl_batchReplyToggle js-batch-reply-toggle${hasReplies ? ' is_clickable' : ' is_zero'}" type="button" aria-expanded="false"${hasReplies ? '' : ' disabled'}>${ICON_COMMENT}${hasReplies ? post.replyCount : ''}</button>
      `;
    }
    ...
    _repliesMarkup(post) {
      ...
      return post.replies.map((reply) => `
        <div class="wl_batchReply">
          <strong class="_author">${this._escapeHTML(reply.author)}</strong>
          <p class="_text">${this._escapeHTML(reply.text)}</p>
          <span class="wl_batchStat${reply.likes ? '' : ' is_zero'}">${ICON_LIKE}${reply.likes}</span>
        </div>
      `).join('') + more;
    }

    // 以 HTML 格式產生 Word 可開啟的 .doc，每篇文章分頁
    _docMarkup(articles) {
      const sections = articles.map((article, index) => {
        const pageBreak = index ? '<br clear="all" style="page-break-before: always">' : '';
        if (article.error) return `${pageBreak}<h1>讀取失敗</h1><p class="meta">${this._escapeHTML(article.url)}</p>`;

        const meta = [article.publisher, article.publishInfo].filter(Boolean).map((text) => this._escapeHTML(text)).join('｜');
        const posts = article.posts.length
          ? `<ol>${article.posts.map((post) => `<li class="post"><b>${this._escapeHTML(post.author)}</b>（讚 ${post.likes ?? '-'}）：${this._escapeHTML(post.text).replace(/\n/g, '<br>')}${this._docRepliesMarkup(post)}</li>`).join('')}</ol>`
          : '<p class="meta">沒有留言</p>';

        return `
          ${pageBreak}
          <h1>${this._escapeHTML(article.title)}</h1>
          <p class="meta">${meta}<br>${this._escapeHTML(article.url)}</p>
          ${article.content.map((text) => `<p>${this._escapeHTML(text)}</p>`).join('')}
          <h2>熱門留言（${article.posts.length}）</h2>
          ${posts}
        `;
      }).join('');
      ...
      return `﻿<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40">
        <head>
          <meta charset="utf-8">
          <title>LINE TODAY 文章匯出</title>
          <style>
            body { font-family: "Microsoft JhengHei", sans-serif; font-size: 12pt; line-height: 1.6; }
            h1 { font-size: 18pt; }
            h2 { margin-top: 24pt; font-size: 14pt; }
            .meta { color: #777; font-size: 10pt; }
            /* Word 會替 p / li / ul 套用自動間距，需同時指定 mso-margin-*-alt 才會照設定；且只認單一 class */
            li.post { margin-top: 6pt; margin-bottom: 0; mso-margin-top-alt: 6pt; mso-margin-bottom-alt: 0; }
            p.replyTitle { margin: 2pt 0 0; mso-margin-top-alt: 2pt; mso-margin-bottom-alt: 0; color: #777; font-size: 10pt; line-height: 1.2; }
            ul.replies { margin-top: 0; margin-bottom: 0; mso-margin-top-alt: 0; mso-margin-bottom-alt: 0; }
            li.reply { margin-top: 0; margin-bottom: 2pt; mso-margin-top-alt: 0; mso-margin-bottom-alt: 2pt; color: #444; font-size: 11pt; line-height: 1.4; }
          </style>
        </head>
        <body>${sections}</body>
      </html>`;
    ...
    // Word：留言底下以縮排清單列出回覆
    _docRepliesMarkup(post) {
      if (!post.replies.length) return '';

      const items = post.replies.map((reply) => `<li class="reply"><b>${this._escapeHTML(reply.author)}</b>（讚 ${reply.likes}）：${this._escapeHTML(reply.text).replace(/\n/g, '<br>')}</li>`).join('');
      return `<p class="replyTitle">回覆（${post.replies.length}${post.replyCount > post.replies.length ? ` / 共 ${post.replyCount}` : ''}）</p><ul class="replies">${items}</ul>`;
    }
